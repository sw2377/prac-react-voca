...
function Day() {
    ...
    const words = useFetch(`http://localhost:3001/words?day=${day}`) // custom hook 사용
    ...
    */
    ...
    // Day 삭제
    function del() {
        if (window.confirm(`Day ${day}를 삭제하시면 Day ${day}에 저장된 모든 단어도 삭제됩니다.`)) {
    ...
    return (
        <div>
            ...
            <div className="btn_flex">
                <button onClick={lastPage} ref={prevRef} 
                    disabled={currentPage === 1 ? true : false}
                >&lt; prev</button>
                <button onClick={nextPage} ref={nextRef}
                    disabled={currentPage >= days.length ? true : false}
                >next &gt;</button>
                <button onClick={del} className="btn_del">Day 삭제</button>
            </div>
